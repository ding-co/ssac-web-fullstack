...
- TDD로 개발? vs. 바로 deadline 맞춰서 개발?
  ...
  - 만약 TDD 아니면 배포할 때도 불안함
    - 사람이 노가다한 것이라 강박증 생김
    - 스트레스 많아짐
- 노드 JS 이벤트 루프 사용 => 동기적 처리
- 알고리즘은 관계가 중요
- B 트리
- 도서 추천
  - 리눅스
    - Unix Bible (명저)
      - 목차 보고 마음에 드는 부분 골라서 읽기
    - 리눅스 시큐리티 쿡북
    - SSH
    - [리눅스 핵심 레퍼런스]
      - 꼭 필요한 명령들만 있음
      - 쉘 스크립트 짜기 위한 기본 자료
      - 프로세스 관리, 모니터링 등 전반적인 이야기
  - [Practical Cloud Security]
    - 운영체제/네트워크 취약점
    - 암호화 키
  - [아파치 카프카 with 자바] (링크드인이 만듦)
    - 메시지 큐
    - DB - 하둡 로깅 - Push성 서버 ex) 메일 서버
    - 보통 3대부터 클러스터링
    - Kafka가 여러 서버 묶어서 한번에 보냄
    - 캐시도 가능
  - 데이터베이스
    - [SQL] 10분
    - 이펙티브 SQL
...
- Serialize vs. De-Serialize
- 아파치 카프카 제로카피, 홀수개 컨센서스?
